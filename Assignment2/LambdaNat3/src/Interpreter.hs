...
evalCBN (EApp e1 e2) = case (evalCBN e1) of
    ...
    e3 -> EApp e3 e2
----------------------------------------------------
--- YOUR CODE goes here for extending the interpreter  WILL ADD 2 LINES
----------------------------------------------------
--EminusOne
evalCBN(EMinusOne (ENatS e)) = evalCBN e --evaluates without successor
evalCBN (EMinusOne (ENat0)) = ENat0 -- 0-1 for successors is just 0

evalCBN(EIf e1 e2 e3 e4) = if (evalCBN e1) ==(evalCBN e2) then (evalCBN e3) else (evalCBN e4)
--want to turn e1 and e2 into a condition
--e1 = S 0
--e2 = EMinusOne (S S 0) --both evaluate to 1 but not the same function
...
evalCBN (ENatS exp3) = ENatS (evalCBN exp3)
evalCBN ENat0 = ENat0
...
--subst id s (ELet i e1 e2) = subst id s (EApp( EAbs i e1) e2) --not sure why this doesnt work.
